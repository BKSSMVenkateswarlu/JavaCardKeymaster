...
public interface KMSEProvider extends KMUpgradable {
   ...
   */
  KMDeviceUniqueKey createDeviceUniqueKey(boolean testMode, byte[] pubKey, short pubKeyOff, short pubKeyLen,
   ...
   */
  KMDeviceUniqueKey getDeviceUniqueKey();
  ...
  /**
   * Read the additional certificate chain data from persistent memory to the output buffer.
   *
   * @param buf    output buffer where the certificate chain is copied.
   * @param offset start offset of the buffer.
   * @return length of the encoded additional certificate chain
   */
  short readAdditionalCertChain(byte[] buf, short offset);
